import React from "react";
import "./facturesPayees.css";

type FacturesPayeesProps = {
    count: number;
    amount: string;
};




const Icon = () => (
    <svg
        className="factures-payees-card__icon"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
    >
        <path d="M7.51 18.35c1.63 1.71 3.8 2.65 6.11 2.65s4.48-.94 6.11-2.65l-1.45-1.38c-1.25 1.31-2.9 2.03-4.66 2.03s-3.41-.72-4.66-2.03c-.55-.58-.99-1.25-1.31-1.97h4.36v-2H7.08c-.04-.33-.07-.66-.07-1s.03-.67.07-1h4.93V9H7.65c.32-.72.76-1.39 1.31-1.97C10.21 5.72 11.86 5 13.62 5s3.41.72 4.66 2.03l1.45-1.38C18.1 3.94 15.93 3 13.62 3s-4.48.94-6.11 2.65C6.59 6.61 5.92 7.75 5.5 9H3v2h2.06c-.03.33-.06.66-.06 1s.02.67.06 1H3v2h2.5a9 9 0 0 0 2.01 3.35" />
    </svg>
);

const FacturesPayees: React.FC<FacturesPayeesProps> = ({
    count,
    amount,
}) => {
    return (
        <div className="factures-payees-card">
            <div className="factures-payees-card__top">
                <Icon />
                <span className="factures-payees-card__label">Factures payées</span>
                <span className="factures-payees-card__count">{count}</span>
            </div>

            <div className="factures-payees-card__bottom">
                <div className="factures-payees-card__amount">{amount}</div>
            </div>
        </div>
    );
};

export default FacturesPayees;