...
import './inputPasswordActuel.css';
...
const InputPasswordActuel: React.FC<PasswordInputProps> = ({ label, id, ...props }) => {
  const [visible, setVisible] = useState(false);
  const inputId = id ?? label?.toLowerCase().replace(/\s+/g, '-');

  return (
    <div className="password-wrapper-actuel">
      {label && (
        <label className="password-label-actuel" htmlFor={inputId}>
          {label}
        </label>
      )}
      <div className="password-field-wrapper-actuel">
        <input
          id={inputId}
          type={visible ? 'text' : 'password'}
          className="password-field-actuel"
          {...props}
        />
        <button
          type="button"
          className="password-toggle-actuel"
...
export default InputPasswordActuel;